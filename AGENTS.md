...
| 2    | Read agent's .md file?          | → Open `.opencode/agents/{agent}.md` |
...
> **CRITICAL:** Before loading any skill or selecting any agent, check `.opencode/profile.json`

```
1. Check if `.opencode/profile.json` exists
2. If EXISTS:
...
python3 .opencode/scripts/checklist.py . --url http://localhost:3000

# Pre-deployment verification
python3 .opencode/scripts/verify_all.py . --url http://localhost:3000

# Kit status
python3 .opencode/scripts/kit_status.py --validate

# Skill management
python3 .opencode/scripts/skills_manager.py list
python3 .opencode/scripts/skills_manager.py search <query>
```
